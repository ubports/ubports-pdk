function initImageVars {
    if [ "$(uname -s)" == "Linux" ]; then
        ...
        if [ "$HOST_ARCH" == "$ARCH" ]; then
            QEMU_ARGS="-enable-kvm -device virtio-vga,virgl=on -display $DISPLAY_ARG,gl=on $QEMU_ARGS"
        else
            QEMU_ARGS="-machine virt -device virtio-gpu-pci,virgl=on -display $DISPLAY_ARG,gl=on $QEMU_ARGS"
        fi
